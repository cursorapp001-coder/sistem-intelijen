...
import { generateGsmLocation } from '../../../services/geminiService';
import { useTool } from '../../useTool';
import { TrackedLocation } from '../../../types';
import { Spinner } from '../../Spinner';
import { LocationResultDisplay } from './LocationResultDisplay';

export const GsmCheck: React.FC = () => {
  const [number, setNumber] = useState('');
  const { isLoading, error, data, execute } = useTool<TrackedLocation, string>(generateGsmLocation);
  ...
  return (
    <div>
      <p className="text-brand-text/80 mb-4">Masukkan nomor telepon untuk melacak lokasi via triangulasi sinyal menara GSM.</p>
